...
for i in events_dic:
    try:
        if time.strptime(events_dic[i]["date_end"],"%Y-%m-%d") > today:
            future_events_tb_list.append((events_dic[i]["event_title"]+" ")*3+ events_dic[i]["description"].decode('ascii', 'ignore'))
...
for i in events_dic:
    try:
        if time.strptime(events_dic[i]["date_end"],"%Y-%m-%d") < today:
            past_events_tb_list.append((events_dic[i]["event_title"]+" ")*3+ events_dic[i]["description"].decode('ascii', 'ignore'))
...
for i in events_dic:
    try:
        events_tb_list.append((events_dic[i]["event_title"]+" ")*3+ events_dic[i]["description"].decode('ascii', 'ignore'))
...
while True:
    detailId = raw_input("Please Select One Event: ")
